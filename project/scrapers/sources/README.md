...
- `facebook_neu_group.json` reads a manual JSON export from the Northeastern Facebook housing group. Facebook does not provide a stable unauthenticated feed for scheduled builds, so `scrapers/manual/facebook_export_browser_console.js` can be run in a logged-in browser to download visible posts as `facebook_group_posts.json`, then the scraper normalizes those posts into the same listing contract.
